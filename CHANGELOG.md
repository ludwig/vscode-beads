...
- **"Details" section in the Repository sidebar** — the left sidebar now caps with a Details section (below Favorites and its end-flourish) that renders the current selection at **medium level-of-detail** via a new reusable `BeadSummaryCard` (type · id · priority · status, title, assignee + label chips, a short description excerpt). It's a normal section — styled header line (not a native twistie) with right-aligned Open-in-tab / Copy-ID / Clear actions — and the summary renders flat inside it (the section is the frame). Clicking it opens the full record in the Secondary Side Bar Details view; a "rule & bead" end-flourish caps the sidebar below it. Replaces the old dark "Selection" card
