...
- **"Details" section in the Repository sidebar** — the left sidebar now caps with a plain Details section (below Favorites and its end-flourish) that renders the current selection at **medium level-of-detail** via a new reusable `BeadSummary` component — a flat field readout (type · id · status · priority, title, type / assignee / label chips, a short description excerpt), no card framing. It's a normal section: styled header line (not a native twistie) with right-aligned actions — **history Back / Forward** (the Details view's old title-bar navigation, now driving the global Details history) plus Open-in-tab / Copy-ID / Clear. A "rule & bead" end-flourish caps the sidebar below it. Replaces the old dark "Selection" card
...
- **Selecting a bead no longer pops the Details sidebar open** — single-click now *selects* (updates the Details content + the panel highlight + the sidebar readout) without revealing, so it never yanks the Secondary Side Bar open when it's closed. **Double-click** reveals/focuses the Details view. Opening in an **editor tab** is now the context-menu "Show in editor tab" + toolbar buttons (+ **Cmd/Ctrl+double-click** in Issues/Kanban, triple-click in Tree/Graph) — double-click no longer opens a tab. Context-menu labels pair as **"Show Details"** (sidebar) / **"Show in editor tab"**. Split the previously-identical `selectBead` (passive) and `openBeadDetails` (reveal) messages to make this possible
- **Details moved to the Secondary Side Bar (right)** — the bead Details view now lives in VS Code's Secondary Side Bar instead of stacked under Repository in the left Beads sidebar, so the left sidebar is navigation/context and details open on the right. Revealing a bead (click in Tree/Kanban/Graph, "Show Details", etc.) auto-opens the Secondary Side Bar. Uses the `secondarySidebar` view-container contribution point (finalized in VS Code 1.106), so the extension's minimum VS Code version is now **1.106** (was 1.85)
- **Details view has a distinct surface in the Secondary Side Bar** — a slightly-elevated background so the Details pane reads as separate from the surrounding IDE chrome (the editor-tab Details keeps the editor background)
