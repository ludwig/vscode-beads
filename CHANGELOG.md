...
- **Sidebar view title tracks the screen** — the merged sidebar view shows the **bead id** in its header while a bead's Details are open, and "Repository" on the Project screen. It's driven by the *screen*, not by selection, so a passive single-click select doesn't oscillate the title (was stuck on "Repository" regardless of screen)
