...
- **History nav arrows in the sidebar** — the Selection card (Project screen) gains clickable ◂ ▸ prev/next arrows next to its Clear button, the visible twin of the existing ⌘←/→ shortcut. On the Details takeover the prev/next history arrows now render in the webview Lead (no longer the native view-title bar), and **Back is never a dead end**: with an earlier issue in the trail it steps back through history, and with none it returns to the Active Project screen (⌘← too). The now-unused `beads.detailScreen` context key is retired
