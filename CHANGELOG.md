...
- **Left sidebar is now a two-screen navigator: Project ↔ Details takeover** — the `beads` sidebar swaps between the **Project** view (active project, a Selection card, actions, Favorites — flush top-to-bottom) and a **full-height Details takeover** (the complete `DetailsView` — description, deps, comments, edit, create) with a **← Back** button. Opening a bead's details (double-click / "Show Details" / clicking the Selection card) swaps to Details; Back returns to Project. Driven by a `beads.detailScreen` context key gating two mutually-exclusive views, so no vertical space-splitting/twisties. Replaces the earlier Secondary-Side-Bar experiment (that container is gone; min VS Code stays 1.85)
...
- **Selection card moved below the action buttons** in the Project view (was above), so the Pick Ready / Show Issues / Show Tree row stays at the top
