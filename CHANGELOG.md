...
- **Selection card in the Project view** — the current selection is captured (above the action buttons) as a medium-LOD `BeadSummary` readout (type · id · status · priority, title, type / assignee / label chips, description excerpt), so it's reachable even after navigating away from the Issues tab. Click it to load the full Details takeover
- **Details Focus toggle acts on the active view immediately + persists** — turning Focus **on** now reveals the current issue in whatever panel tab is already open (no tab switch), instead of waiting for a Show-tab click. If no panel/tab is open it's a no-op — the focus-mode Show buttons still reveal on click. The Focus state is now persisted, so it survives navigating between issues, screen swaps, and reloads
- **History nav arrows on both sidebar screens' title bar** — the ◂ ▸ prev/next arrows now show in the view-title bar on *both* the Repository and Details screens (the visible twin of ⌘←/→), each screen wiring them to its own command so there's no surface-sniffing. On the **Details** screen, **Back** is no longer a dead end on a freshly-opened issue: it steps back through the trail and, once empty, returns to the Active Project screen (`beads.sidebarNavigateBack`; ⌘← too). On the **Repository** screen, **Forward** re-opens the issue you backed out of (`beads.sidebarProjectForward`, sharing the provider's ⌘→ logic) and Back reuses the plain history nav. The pure editor-tab history commands (`beads.navigateBack/Forward`) are untouched
- **"Show Tree" sidebar action** — a new button next to "Show Issues" (sharing its steel-blue accent) reveals the Beads panel with the Tree tab focused, mirroring the existing Show-Issues/Show-Kanban focus flow. The sidebar action row is ordered Pick Ready Bead · Show Issues · Show Tree
- **"Copy ID" on a Favorites card's right-click menu** — copies the bead's ID to the clipboard (first item in the menu), alongside the existing Show Details / Open in editor tab
- **The common filter surface is now linked across all panel views** — toggling Favorites/Ready, picking a preset, or adding a status/priority/type/label chip in *any* panel view (Issues, Kanban, Tree, Graph) updates one shared filter the host owns and echoes back, so the views stay in lockstep. The Issues tab is the canonical **leader** (it owns and publishes its filter, and on cold load seeds the shared spec with its `¬closed` default); Kanban/Tree/Graph are live **followers** that render the shared spec and publish edits back. When you switch back to Issues, it re-seeds from the shared spec so a change made elsewhere is picked up. Each view keeps its own free-text search local; the collapse state is now synced across the panel tabs. Editor-tab views stay divergent. *(Race-free re-land of the earlier linking attempt: the leader seeds once at mount rather than live-consuming, and no fabricated default competes at cold load.)*
...
- **Details header reworked into a Lead + Ident layout** — a top *Lead* line carries context on the left (a labeled **← Project** back button in the sidebar takeover, or an "Issue view for &lt;project&gt;" chrome label in an editor tab) and, on the right, the **action cluster** (sidebar) or the back/forward history nav (editor tab, which has no view-title bar to host it). The *Ident* line below holds the type icon + ID + **favorite star**, with the merged **type · status · priority** pill right-aligned — same joined style as the dependency lists; the inline-editable selects appear in the badges row while editing
- **Sidebar view title tracks the screen** — the merged sidebar view header reads a categorical **"Issue Details"** while a bead's Details are open, and **"Repository"** on the Project screen (VS Code uppercases both). It's driven by the *screen*, not by selection, so a passive single-click select doesn't oscillate the title (was stuck on "Repository" regardless of screen)
- **Restored the sidebar Details back/forward nav** — the ◄ ► buttons return to the view-title bar (Pre-Lead), enabled by the global history. Clicking a child/dependency link in the Details pane now *records* a navigation step (the sidebar's global trail, an editor tab's per-tab trail), so the arrows are meaningful as you click around relatives
- **Grouped "show in tab" buttons** replace the awkward "Show in ▾" split-dropdown in the Details header — one icon button each for Issues / Tree / Kanban / Graph
- **Selection card: only the title is a link** — the medium-LOD `BeadSummary` no longer captures clicks on the whole card (its text is now selectable); the **title** is the link that opens the full Details view. Its pills are right-aligned and merged, the redundant "Type" row is gone, and relationship/comment counts sit below the Description/Acceptance-Criteria sections (mirroring the full view's flow)
...
- **Closed-issue titles in the Selection readout are muted, not struck through** — the medium-LOD `BeadSummary` now dims closed titles (matching the Issues table / cards) instead of applying a strikethrough
- **Selecting a bead no longer forces the Details view open** — single-click now *selects* (updates the Details content + the panel highlight + the Selection card) without swapping the sidebar to the Details takeover. **Double-click** opens the takeover. Opening in an **editor tab** is the context-menu "Show in editor tab" + toolbar buttons (+ **Cmd/Ctrl+double-click** in Issues/Kanban, triple-click in Tree/Graph) — double-click no longer opens a tab. Context-menu labels pair as **"Show Details"** / **"Show in editor tab"**. Split the previously-identical `selectBead` (passive) and `openBeadDetails` (reveal) messages to make this possible
...
- **Sidebar text sizes use the design tokens (no more sub-floor hardcoded px)** — the new Details readout, the Kanban priority chip, and the collapsed-ribbon filter pills were setting hardcoded font sizes below the 11px token floor (down to 8–9.5px), which read as tiny and didn't scale with the user's VS Code font size. They now use the `--text-label`/`--text-sm`/`--text-body` and `--badge-font-*` tokens; the ribbon's Ready pill uses the lucide `Rocket` icon (matching the Favorites star) instead of a hand-sized emoji glyph
- **Filter funnel no longer nudges when you collapse/expand the bar** — the funnel lives in the (borderless) search row when expanded and in the (bottom-bordered) ribbon when collapsed; the 1px border difference shifted its centered position by ~½px. The search row now reserves a matching transparent bottom border, so the funnel stays put
- **Follower views now match the Issues count for favorites/ready filters** — the host's shared-scope resolution used a lazily-primed edge cache that could still be cold when it ran, so `favoritesWithRelatives` dropped the 1-hop relatives (and "ready" degenerated), leaving Kanban/Tree/Graph showing a smaller set than the Issues view (which resolves locally from the loaded graph). Fetching the dependency graph now warms the host edge cache with the same graph the webview gets and recomputes the shared scope, so all four views agree
- **Preset dropdown no longer sticks on "Custom"** — the dropdown label is now derived purely from the status column (via `matchStatusPreset`) instead of a stored `activePreset` string. A follower tab could inherit a foreign/stale preset id (the `"custom"` sentinel from an empty shared spec), which left the dropdown reading "Custom" even with no status filter — and toggling Favorites/Ready wouldn't shake it loose. An empty status set now always reads as "All", an exact preset-set match reads as that preset, and only a genuinely unmatched status set reads as "Custom" (regression-covered)
- **Ready pill in the collapsed ribbon shows its 🚀** — the Ready token in the ribbon summary was missing the rocket glyph the Favorites token's ★ already had; both filter pills now carry their icon
