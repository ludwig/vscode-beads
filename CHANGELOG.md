...
- **The common filter surface is now linked across all panel views** — toggling Favorites/Ready, picking a preset, or adding a status/priority/type/label chip in *any* panel view (Issues, Kanban, Tree, Graph) updates one shared filter the host owns and echoes back to every panel view, so they stay in lockstep ("shared base + local overlay"). Each view keeps its own free-text search and collapse state local. Editor-tab views stay divergent (a local filter seeded from the shared base, pushed explicitly via "Apply to all"). Because a shared filter now exists whenever any common control is set, the inherited-scope **"Filtered" toggle** appears on every panel ribbon
