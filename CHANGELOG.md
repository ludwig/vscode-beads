...
- **"Show Tree" sidebar action** — a new button next to "Show Issues" (sharing its steel-blue accent) reveals the Beads panel with the Tree tab focused, mirroring the existing Show-Issues/Show-Kanban focus flow. The sidebar action row is now ordered Show Issues · Show Tree · Pick Ready Bead
...
- **Preset dropdown flips back to "All" when the filter is emptied** — removing the last status chip individually left the dropdown stuck on "Custom"; it now resolves to "All" whenever no column filters remain (and the matching menu item is marked active)
