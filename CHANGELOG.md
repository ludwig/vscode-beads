...
- **"Details" section in the Repository sidebar** — the left sidebar now has a Details section (styled header line, not a native twistie; right-aligned Open-in-tab / Copy-ID / Clear actions) that renders the current selection at **medium level-of-detail** via a new reusable `BeadSummaryCard` (type · id · priority · status, title, assignee + label chips, a short description excerpt). Clicking it opens the full record in the Secondary Side Bar Details view. Replaces the old dark "Selection" card
