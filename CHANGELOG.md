...
- **Details header reworked into a leading line + identity/actions row** — a top *leading line* carries context on the left and a right-aligned control: in the **sidebar takeover** it's a labeled **← Project** back button + the merged **type · status · priority** pill; in an **editor tab** it's an "Issue view for &lt;project&gt;" chrome label + the back/forward history nav (moved off the crowded action cluster). Below it, the identity row (type icon + ID) holds the action cluster (favorite / refresh / + / edit / show-in-tab / open-in-tab). The merged pill uses the same joined style as the dependency lists; the inline-editable selects appear in the badges row while editing
