...
- **Sidebar history nav arrows + Back-to-project** — the Selection card (Project screen) gains clickable ◂ ▸ prev/next arrows next to Clear, the visible twin of the existing ⌘←/→ shortcut. On the Details takeover the title-bar **Back** is no longer a dead end on a freshly-opened issue: it steps back through the issue trail and, once empty, returns to the Active Project screen (⌘← too). It runs a dedicated `beads.sidebarNavigateBack` command so the pure editor-tab history nav (`beads.navigateBack`) is untouched
