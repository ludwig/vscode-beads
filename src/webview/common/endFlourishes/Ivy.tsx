/**
 * Ivy — a full-width hairline that, at each end, breaks into a climbing ivy
 * tendril: a short shoot curling UP and a longer one curling DOWN (≈1:2), each
 * tipped with a leaf. Built as [leaf cap][flexible rule][leaf cap] so the rule
 * spans the entire width while the caps stay a fixed size, hugging the edges.
 * Part of the end-flourish library (see ./index).
 *
 * Geometry note: the rule enters each cap at y=20 of a 0..60 viewBox (one third
 * down), so the foliage reaches ~15 units up and ~30 down — the line's
 * margin-top in CSS (.view-end-line) is tuned to that same one-third entry.
 */
...
export function Ivy(): React.ReactElement {
  return (
